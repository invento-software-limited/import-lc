	...
	def validate_one_to_one(self):
		if self.purchase_order:
			existing_pi = frappe.db.get_value(
				"Proforma Invoice",
				{"purchase_order": self.purchase_order, "name": ["!=", self.name], "docstatus": ["<", 2]},
				"name",
			)
			if existing_pi:
				...
				frappe.throw(
					f"Proforma Invoice <b>{link}</b> already exists for Purchase Order <b>{po_link}</b>"
				)
...
@frappe.whitelist()
def make_proforma_invoice(source_name: str, target_doc: str | None = None):
...
@frappe.whitelist()
def make_import_insurance(source_name: str, target_doc: str | None = None):
...
@frappe.whitelist()
def make_import_lc(source_name: str, target_doc: str | None = None):
