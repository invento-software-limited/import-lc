...
@frappe.whitelist()
def make_purchase_invoice(source_name: str, target_doc: str | None = None):
...
@frappe.whitelist()
def make_purchase_receipt(source_name: str, target_doc: str | None = None):
	...
	def set_missing_values(source, target):
		...
		pi_name = frappe.db.get_value(
			"Purchase Invoice", {"import_lc": source.name, "docstatus": ["<", 2]}, "name"
		)
		if pi_name:
			...
			fields_to_map = [
				...
				"safta_clause",
				"mode_of_transport",
				"mode_of_shipment_container_details",
				"shipment_conditions",
				"country_of_origin",
			]
...
@frappe.whitelist()
def make_journal_entry(source_name: str):
...
@frappe.whitelist()
def make_lc_expense_journal_entry(source_name: str):
...
@frappe.whitelist()
def make_landed_cost_voucher(source_name: str, source_doctype: str = "Import LC"):
	...
	# Fetch PRs
	prs = frappe.get_all(
		"Purchase Receipt", filters=filters, fields=["name", "supplier", "posting_date", "base_grand_total"]
	)
	for pr in prs:
		lcv.append(
			"purchase_receipts",
	...
	if lc.import_insurance:
		...
		pi_name = frappe.db.get_value(
			"Purchase Invoice",
			{"import_insurance": lc.import_insurance, "purchase_type": "Local", "docstatus": 1},
			"name",
		)
		...
		if pi_name:
			...
			pi_data = frappe.db.get_value(
				"Purchase Invoice",
				pi_name,
				["name", "supplier", "posting_date", "base_grand_total"],
				as_dict=1,
			)
			...
			if pi_data:
				...
				lcv.append(
					"vendor_invoices", {"vendor_invoice": pi_data.name, "amount": pi_data.base_grand_total}
				)
	...
	if not jes:
		...
		if jes:
			...
			for d in jes:
				d.import_lc = d.import_lc

	# je_info = [
	# 	f"{d.name} ({d.voucher_type}) - Amt: {d.get('total_amount') or d.get('total_debit')}" for d in jes
	# ]
	...
	if total_lc_expenses:
		...
		lcv.append(
			"taxes", {"description": desc, "amount": total_lc_expenses, "base_amount": total_lc_expenses}
		)

	if freight:
		lcv.append("taxes", {"description": "Freight Charges", "amount": freight, "base_amount": freight})
	...
	if insurance:
		...
		if lc.import_insurance:
			...
			pi_name = frappe.db.get_value(
				"Purchase Invoice", {"import_insurance": lc.import_insurance, "docstatus": 1}, "name"
			)
			if pi_name:
				desc += f" (Invoice: {pi_name})"

		lcv.append(
			"taxes", {"description": desc, "amount": insurance, "base_amount": insurance, "account": account}
		)
	...
	if total_margin:
		...
		lcv.append("taxes", {"description": desc, "amount": total_margin, "base_amount": total_margin})
	...
	if lcv.purchase_receipts:
		...
		for dtype, names in receipt_docs.items():
			...
			stock_items_info = frappe.get_all(
				"Item",
